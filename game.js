...
const SOLDIER_MODEL_URL = './soldier.glb';
const WEAPON_MODEL_URL = './weapon.glb';
const CHAR_TURN_RATE = 3.0;       // rad/s — يدور الجندي أسرع من الدبابة
const CHAR_RUN_SPEED = 6.2;       // m/s
const CHAR_REV_SPEED = 2.4;       // m/s
const CHAR_ACCEL = 16, CHAR_DECEL = 20;
const CHAR_RADIUS = 0.42;
const CHAR_EYE_HEIGHT = 1.65;
const RIFLE_RANGE_GRAVITY = 0.10; // انحناء بسيط جداً لمسار الطلقة
const RIFLE_FIRE_RATE = 0.13;     // ثانية بين كل طلقة أثناء الضغط المستمر
const RIFLE_DAMAGE_ENEMY = 7;
const BULLET_SPEED = 150;

let scoreCount = 0;
let playerHealth = PLAYER_MAX_HEALTH;
let gameOver = false;
let controlMode = 'tank'; // 'tank' | 'soldier'
...
function restartGame() {
  ...
  charYaw = Math.PI; charSpeed = 0; rifleFiring = false;
  ...
  const cp = randomPointNear(0, 0, 5, 8);
  const nb = nearbyEntries(cp, 12);
  charGroup.position.set(cp.x, groundHeightAt(cp, nb), cp.z);
...
const loadState = { tank: 0, city: 0, soldier: 0, weapon: 0, tankTotal: 1, cityTotal: 1, soldierTotal: 1, weaponTotal: 1 };

function updateLoadUI() {
  const t = loadState.tank / (loadState.tankTotal || 1);
  const c = loadState.city / (loadState.cityTotal || 1);
  const s = loadState.soldier / (loadState.soldierTotal || 1);
  const w = loadState.weapon / (loadState.weaponTotal || 1);
  const pct = Math.round(((t + c + s + w) / 4) * 100);
...
/* ============================================================
   الجندي والسلاح
============================================================ */
const charGroup = new THREE.Object3D();
scene.add(charGroup);
let charModel = null, weaponModel = null, weaponBone = null;
let hipL = null, hipR = null, kneeL = null, kneeR = null;
let charYaw = Math.PI, charSpeed = 0;
let walkPhase = 0;
let rifleFiring = false, rifleCooldownLeft = 0;

function setupSoldier(gltf, weaponGltf) {
  charModel = gltf.scene;
  charModel.traverse((o) => {
    if (o.isMesh) { o.castShadow = true; o.receiveShadow = true; }
    if (/^bip_hip_L/.test(o.name)) hipL = o;
    if (/^bip_hip_R/.test(o.name)) hipR = o;
    if (/^bip_knee_L/.test(o.name)) kneeL = o;
    if (/^bip_knee_R/.test(o.name)) kneeR = o;
    if (/^weapon_bone(?!.*_L_)(?!.*end)/i.test(o.name)) weaponBone = o;
  });

  const box0 = new THREE.Box3().setFromObject(charModel);
  const rawHeight = box0.max.y - box0.min.y;
  const TARGET_HEIGHT = 1.8; // متر — طول إنسان واقعي
  const SOLDIER_EXTRA_SCALE = 1; // ⬅️ لو ظل يبين صغير، جرب ترفعه لـ 1.5 أو 2 وشوف الفرق
  const scaleFactor = (rawHeight > 0.001 ? THREE.MathUtils.clamp(TARGET_HEIGHT / rawHeight, 0.02, 50) : 1) * SOLDIER_EXTRA_SCALE;
  charModel.scale.setScalar(scaleFactor);

  const box = new THREE.Box3().setFromObject(charModel);
  charModel.position.y = -box.min.y;
  charGroup.add(charModel);

  weaponModel = weaponGltf.scene;
  weaponModel.traverse((o) => { if (o.isMesh) { o.castShadow = true; } });
  if (weaponBone) {
    weaponBone.add(weaponModel);
    // إزاحة تقريبية لمحاذاة قبضة السلاح مع اليد — عدّل هذه القيم لو احتاج الأمر
    weaponModel.position.set(0, 0, 0);
    weaponModel.rotation.set(0, 0, 0);
  } else {
    // إن لم يوجد عظم مخصص للسلاح، ألصقه بمقدمة الشخصية كحل احتياطي
    charGroup.add(weaponModel);
    weaponModel.position.set(0.25, 1.3, 0.3);
  }
}

function forwardVectorYaw(yaw, out) {
  out.set(Math.sin(yaw), 0, Math.cos(yaw));
  return out;
}
...
let lastCharNearby = [];
let charNearbyTimer = 0;

function updateSoldier(dt) {
  charYaw += input.steer * CHAR_TURN_RATE * dt;
  const targetSpeed = input.throttle >= 0 ? input.throttle * CHAR_RUN_SPEED : input.throttle * CHAR_REV_SPEED;
  const rate = Math.abs(targetSpeed) > Math.abs(charSpeed) ? CHAR_ACCEL : CHAR_DECEL;
  charSpeed += Math.sign(targetSpeed - charSpeed) * Math.min(Math.abs(targetSpeed - charSpeed), rate * dt);
  if (Math.abs(input.throttle) < 0.01 && Math.abs(charSpeed) < 0.05) charSpeed = 0;

  forwardVectorYaw(charYaw, _charFwd);
  const proposed = charGroup.position.clone().addScaledVector(_charFwd, charSpeed * dt);

  charNearbyTimer -= dt;
  if (charNearbyTimer <= 0) { lastCharNearby = nearbyEntries(charGroup.position, 22); charNearbyTimer = 0.15; }
  const resolved = resolveCircleCollision(proposed, CHAR_RADIUS, lastCharNearby);
  const gY = groundHeightAt(resolved, lastCharNearby);
  charGroup.position.set(resolved.x, gY, resolved.z);
  charGroup.rotation.y = charYaw;
  charGroup.updateMatrixWorld(true);

  const moving = Math.abs(charSpeed) > 0.15;
  const targetAmp = moving ? Math.min(1, Math.abs(charSpeed) / CHAR_RUN_SPEED) : 0;
  if (moving) walkPhase += (charSpeed * dt) * 3.4;
  if (hipL && hipR && kneeL && kneeR) {
    const swing = Math.sin(walkPhase) * 0.5 * targetAmp;
    hipL.rotation.x = swing;
    hipR.rotation.x = -swing;
    kneeL.rotation.x = Math.max(0, Math.sin(walkPhase + Math.PI * 0.5)) * 0.7 * targetAmp;
    kneeR.rotation.x = Math.max(0, Math.sin(walkPhase - Math.PI * 0.5)) * 0.7 * targetAmp;
  }

  speedVal.textContent = Math.round(Math.abs(charSpeed) * 3.6);

  if (rifleFiring) {
    rifleCooldownLeft -= dt;
    if (rifleCooldownLeft <= 0 && !gameOver) { rifleCooldownLeft = RIFLE_FIRE_RATE; fireRifle(); }
  }
}

function fireRifle() {
  const muzzle = new THREE.Vector3();
  if (weaponBone) weaponBone.getWorldPosition(muzzle); else charGroup.getWorldPosition(muzzle);
  const fwd = forwardVectorYaw(charYaw, new THREE.Vector3());
  muzzle.y += CHAR_EYE_HEIGHT * 0.62;
  muzzle.addScaledVector(fwd, 0.5);
  const mesh = new THREE.Mesh(bulletGeo, bulletMat);
  mesh.position.copy(muzzle);
  scene.add(mesh);
  shells.push({ mesh, vel: fwd.clone().multiplyScalar(BULLET_SPEED), life: 1.8, owner: 'player', gravityScale: RIFLE_RANGE_GRAVITY, damage: RIFLE_DAMAGE_ENEMY });
...
/* ============================================================
   وضع التحكم: دبابة / جندي
============================================================ */
function setControlMode(mode) {
  controlMode = mode;
  input.steer = 0; input.throttle = 0;
  rifleFiring = false;
  if (modeLabel) modeLabel.textContent = mode === 'tank' ? '🚗 الدبابة' : '🪖 الجندي';
  toast(mode === 'tank' ? 'تم التحويل لقيادة الدبابة' : 'تم التحويل للتحكم بالجندي', 2000);
}
function toggleControlMode() {
  if (controlMode === 'tank' && !soldierLoaded) {
    toast('لم يتم تحميل نموذج الجندي بعد أو فشل تحميله — تحقق من ملفي soldier.glb و weapon.glb', 3200);
    return;
  }
  setControlMode(controlMode === 'tank' ? 'soldier' : 'tank');
}
function getPlayerPosition() {
  return controlMode === 'tank' ? tankGroup.position : charGroup.position;
}
...
const bulletMat = new THREE.MeshBasicMaterial({ color: 0xfff2b0 });
const shells = [];
...
function updateShells(dt) {
  for (let i = shells.length - 1; i >= 0; i--) {
    const s = shells[i];
    s.vel.y += GRAVITY * (s.gravityScale != null ? s.gravityScale : 1) * dt;
    ...
    if (!dead && s.owner === 'player') {
      for (const en of enemies) {
        ...
        if (dx * dx + dy * dy + dz * dz < 2.5 * 2.5) {
          en.health -= (s.damage != null ? s.damage : 22);
          hitEffects.push(spawnHitEffect(s.mesh.position.clone()));
          dead = true;
...
function setupActionButtons() {
  ...
  fireBtn.addEventListener('pointerup', () => { rifleFiring = false; });
  fireBtn.addEventListener('pointerleave', () => { rifleFiring = false; });
  fireBtn.addEventListener('pointercancel', () => { rifleFiring = false; });
  ...
  canvas.addEventListener('pointerdown', (e) => {
    if (editMode || controlMode !== 'tank') return;
...
function updateCamera(dt) {
  if (controlMode === 'soldier') { updateSoldierCamera(dt); return; }
...
function updateSoldierCamera(dt) {
  const fwd = forwardVectorYaw(charYaw, tmpVec);
  let dist, height, lookHeight;
  if (input.aiming) { dist = 2.0; height = 1.65; lookHeight = 1.55; } else { dist = 3.2; height = 1.9; lookHeight = 1.45; }
  const desired = charGroup.position.clone().addScaledVector(fwd, -dist).add(new THREE.Vector3(0, height, 0));
  camPos.lerp(desired, 1 - Math.pow(0.0004, dt));
  const lookTarget = charGroup.position.clone().addScaledVector(fwd, 6).add(new THREE.Vector3(0, lookHeight, 0));
  camLook.lerp(lookTarget, 1 - Math.pow(0.0006, dt));
  camera.position.copy(camPos);
  camera.lookAt(camLook);
  const baseFov = input.aiming ? 42 : 58;
  const targetFov = baseFov * ZOOM_FOVS[input.zoomLevel];
  camera.fov += (targetFov - camera.fov) * Math.min(1, dt * 6);
  camera.updateProjectionMatrix();
}
...
function animate() {
  ...
  if (!gameOver) {
    if (controlMode === 'tank') updateTank(dt); else updateSoldier(dt);
    const playerPos = getPlayerPosition();
    for (const e of enemies) updateEnemy(e, dt, playerPos);
  }
  updateShells(dt);
  updateCamera(dt);
...
async function boot() {
  ...
  try {
    ...
    loadSub.textContent = 'تحميل الجندي…';
    try {
      const soldierGltf = await loadGLB(SOLDIER_MODEL_URL, (l, t) => { loadState.soldier = l; loadState.soldierTotal = t; updateLoadUI(); });
      loadSub.textContent = 'تحميل السلاح…';
      const weaponGltf = await loadGLB(WEAPON_MODEL_URL, (l, t) => { loadState.weapon = l; loadState.weaponTotal = t; updateLoadUI(); });
      setupSoldier(soldierGltf, weaponGltf);
      const p = randomPointNear(tankGroup.position.x, tankGroup.position.z, 5, 8);
      const nearbySpawn = nearbyEntries(p, 12);
      const gYc = groundHeightAt(p, nearbySpawn);
      charGroup.position.set(p.x, gYc, p.z);
      charGroup.rotation.y = charYaw;
      soldierLoaded = true;
      console.log('[تحقق] تم تحميل الجندي ووضعه عند:', charGroup.position, 'حجم الصندوق المحيط:', new THREE.Box3().setFromObject(charGroup), 'عامل التحجيم:', charModel.scale.x);
    } catch (soldierErr) {
      soldierLoaded = false;
      console.error('فشل تحميل الجندي أو السلاح — تأكد من رفع soldier.glb و weapon.glb لنفس مجلد الريبو:', soldierErr);
      loadSub.textContent = 'تحذير: تعذر تحميل الجندي (تحقق من ملفي soldier.glb و weapon.glb) — الدبابة تعمل بشكل طبيعي.';
    ...
    console.error(err);
    loadSub.textContent = 'حدث خطأ أثناء تحميل النماذج. تأكد أن كل الملفات (game.js, tank.glb, city_map.glb, soldier.glb, weapon.glb) في نفس المجلد وأن الصفحة تُفتح عبر خادم محلي.';
  }
}
